...
.kpi { border-bottom: 5px solid #0d9488; }
...
@media(max-width: 950px){ .lay { flex-direction: column; } .side { width: 100%; position: static; } .g2, .g3 { grid-template-columns: 1fr; } .fgrid{ grid-template-columns: 1fr; } .report-header { flex-direction: column; gap: 8px; } }
    ...
    <div className="rw" style={{ direction: t.dir, fontFamily: t.font }}>
      ...
      <div className="lay">
        ...
        <div className="main">
                ...
                {dashRoomData.map((r, i) => (
                  <div key={r.id} style={{ marginBottom: "14px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px", fontSize: "14px" }}>
                      ...
                      <span style={{ color: i === 0 ? TEAL : "#334155", fontWeight: 900, direction: "ltr" }}>{r.avg.toFixed(2)}</span>
                    </div>
          ...
          {tab === "final" && (
            <div>
              <div className="card" style={{ borderRight: "6px solid " + TEAL }}>
                <h1 style={{ fontSize: "28px", fontWeight: 900, marginBottom: "8px", color: TEAL_DARK }}>{t.tab3}</h1>
              ...
              {partRoomData.map((r) => (
                <div key={r.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
                  <div style={{ background: "#f8fafc", padding: "16px", borderBottom: "1px solid #e2e8f0", display: "flex", gap: "16px", alignItems: "center" }}>
                    <b style={{ fontSize: "18px", color: TEAL }}>{(lang === "ar" ? "قاعة " : "Room ") + r.code}</b>
          ...
          {tab === "cert" && (
            <div>
              <div className="card" style={{ borderRight: "6px solid " + GOLD }}>
                <h1 style={{ fontSize: "28px", fontWeight: 900, marginBottom: "8px", color: GOLD }}>{t.tab5}</h1>
              ...
              {certBest ? (
                <div className="card" style={{ textAlign: "center", padding: "40px", background: "linear-gradient(135deg, #fffbeb, #fef3c7)", border: `4px solid ${GOLD}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                    <img src="/logo-upm.png" alt="UPM" style={{ height: 64 }} />
                    <img src="/logo-center.png" alt="Center" style={{ height: 64 }} />
                  </div>
                  <h1 style={{ fontSize: "36px", fontWeight: 900, color: GOLD, marginBottom: "8px" }}>{lang === "ar" ? "شهادة تميز وإشادة" : "Certificate of Excellence"}</h1>
                  <p style={{ color: "#64748b", marginBottom: "24px" }}>{lang === "ar" ? "يمنح هذا التكريم لأفضل مدرب" : "This award is granted to the best trainer"}</p>
                  <h2 style={{ fontSize: "48px", fontWeight: 900, color: TEAL, marginBottom: "16px" }}>{certBest.trainer}</h2>
                  <p style={{ fontSize: "20px", color: "#334155" }}>
                    {lang === "ar" ? "قاعة" : "Room"} <b style={{ color: BLUE }}>{certBest.code}</b> — {lang === "ar" ? "معدل" : "Average"}:{" "}
                    <b style={{ color: GOLD, direction: "ltr" }}>{certBest.avg.toFixed(2)}/5</b> ({lang === "ar" ? "استجابات" : "responses"}: {certBest.cnt})
                  </p>
