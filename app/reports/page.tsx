...
const RECS={
  trainer:{ar:"رفع التفاعل داخل القاعة، استخدام أمثلة تطبيقية، تخصيص وقت للأسئلة، وتلخيص أهم النقاط نهاية الحصة.",en:"Increase engagement, practical examples, Q&A time, end-of-session summary."},
  education:{ar:"مراجعة وضوح المحتوى وملاءمته للمستوى، إضافة أنشطة تطبيقية قصيرة، وربط المحتوى بمخرجات التعلم.",en:"Review content clarity/level fit, add short activities, align with outcomes."},
  facility:{ar:"فحص الصوت والعرض والإنترنت قبل الجلسة، وتفعيل قناة سريعة للإبلاغ عن الأعطال.",en:"Pre-check AV/internet; fast issue-reporting channel."},
  arrival:{ar:"تأكيد ترتيبات الوصول قبل 24 ساعة، تعليمات موحدة، مسؤول ميداني، ورقم دعم فوري.",en:"Confirm arrivals 24h ahead, unified instructions, field coordinator, live support."},
  hospitality:{ar:"توحيد معايير الضيافة، تقليل الانتظار، ومتابعة الجودة بقياس يومي.",en:"Standardize hospitality, reduce waiting, daily quality monitoring."},
  transport:{ar:"تثبيت جدول النقل، توضيح نقاط التجمع، تنبيهات قبل الرحلات، ودعم عند التأخر.",en:"Stable transport schedule, clear meeting points, reminders, delay support."},
  housing:{ar:"فحص دوري للنظافة والراحة، قناة بلاغات، وقياس سرعة الاستجابة.",en:"Regular cleanliness checks, reporting channel, response-time tracking."},
  tours:{ar:"تحسين تنظيم الجولات وضبط الوقت، وقياس رضا كل نشاط بشكل مستقل.",en:"Improve tour organization/timing; measure satisfaction per activity."},
  umrah:{ar:"توضيح خطة العمرة والخدمات، رفع الإرشاد الميداني، ودعم فوري للمشاركين.",en:"Clarify Umrah plan/services, field guidance, immediate support."},
  administration:{ar:"تحسين وضوح التعليمات، توحيد التواصل، مسؤول خدمة واضح، ومتابعة الملاحظات حتى الإغلاق.",en:"Clarify instructions, unify communication, assign owner, track to closure."},
  satisfaction:{ar:"تحليل مسببات الرضا عبر المحاور الأخرى ومعالجة الأعلى أثرًا أولًا.",en:"Analyze drivers across axes; act on highest impact first."},
  general:{ar:"تنفيذ تحسينات سريعة ثم إعادة القياس خلال أسبوعين.",en:"Apply quick fixes; re-measure within two weeks."}
};
function recFor(ar,en,riskKey,lang){
  if(riskKey==="NA")return lang==="ar"?"العينة صغيرة؛ يفضّل زيادة الاستجابات قبل اتخاذ قرار.":"Small sample; collect more first.";
  if(riskKey==="LOW")return lang==="ar"?"نقطة قوة: المحافظة على الممارسة وتوثيقها كنموذج.":"Strength: maintain and document.";
  return RECS[axisCategory(ar,en)]?.[lang]||RECS.general[lang];
}
function buildAxisStats(ca,lang){
  ...
  for(const g of groups.values()){
    ...
    out.push({axisAr:g.ar,axisEn:g.en,axisLabel:lang==="ar"?g.ar:g.en,...s,risk,recommendation:recFor(g.ar,g.en,risk.key,lang)});
  }
...
function MetricCard({title,value,sub,color=BLUE,icon="📊"}){
  return(
    <div className="mcard" style={{borderBottomColor:color}}>
      <div className="mtop"><span>{icon}</span><span>{title}</span></div>
      ...
      <div className="twrap">
        <table className="tbl">
          <thead><tr>
            ...
            <th className="th">{isAr?"المتوسط":"Mean"}</th><th className="th">{isAr?"الوسيط":"Median"}</th>
            <th className="th">{isAr?"التباين":"Var"}</th><th className="th">{isAr?"الانحراف":"SD"}</th>
            <th className="th">{isAr?"منخفض":"Low"}</th><th className="th">{isAr?"مرتفع":"High"}</th>
            <th className="th">{isAr?"الحالة":"Risk"}</th><th className="th">{isAr?"التوصية":"Recommendation"}</th>
          </tr></thead>
          <tbody>
            {items.map((x,i)=>(
              <tr key={i}>
          ...
          {tab==="dashboard"&&(
            <div>
              ...
              <div className="g4">
                ...
                <MetricCard icon="⚠️" color={dashHighRisk?RED:TEAL} title={isAr?"محاور القلق":"Risk Areas"} value={dashHighRisk} sub={isAr?"تحتاج تدخلًا":"Need action"}/>
                <MetricCard icon="📉" color={ORANGE} title={isAr?"منخفض 1–2":"Low 1–2"} value={`${dashSummary.lowPct.toFixed(0)}%`} sub={isAr?"نسبة عدم الرضا":"Dissatisfaction"}/>
              </div>
              ...
              <div className="g2">
                <div className="card"><h3 className="ctitle">{isAr?"📊 توزيع التقييمات":"📊 Rating Distribution"}</h3><DistributionChart stats={dashSummary} lang={lang}/></div>
                <div className="card"><h3 className="ctitle">{isAr?"📈 الاتجاه — آخر 7 أيام بيانات":"📈 Trend — Last 7 Data Days"}</h3><TrendChart data={dashTrend} lang={lang} color={BLUE}/></div>
              </div>
