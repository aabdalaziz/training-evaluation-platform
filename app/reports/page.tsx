...
export default function ReportsPage() {
  ...
  const partRoomData = useMemo(() => {
    ...
    for (const c of classrooms) {
      ...
      // respect classroom filter (if set)
      if (partF.classroomId !== "ALL" && c.id !== partF.classroomId) continue;
      // respect trainer filter (if set)
      if (partF.trainerId !== "ALL" && c.trainer_id !== partF.trainerId) continue;
    ...
    return out;
  }, [partRowsF, classrooms, trainers, partF.q, partF.classroomId, partF.trainerId]);
  ...
  const certBest = useMemo(() => {
    // compute avg by classroom, apply min responses
    const map = new Map();
  ...
  const FiltersBar = ({ value, setValue, withQuery = false, withMin = false }) => {
    return (
      <div className="card noprint" style={{ padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <span className="badge" style={{ background: "#eef2ff", color: "#3730a3" }}>
            {t.sampleHint}: {value?.kind === "DAILY" ? daily.count : value?.kind === "FINAL" ? final.count : rows.length}
          </span>
        </div>
        ...
        <div className="fgrid">
          ...
          <select className="sel" value={value.classroomId} onChange={(e) => setValue((f) => ({ ...f, classroomId: e.target.value }))}>
          ...
          {withQuery && (
            <input className="inp" placeholder={t.search} value={value.q} onChange={(e) => setValue((f) => ({ ...f, q: e.target.value }))} />
          )}
        ...
        <div className="main">
          ...
          {tab === "dashboard" && (
            <div>
              ...
              <FiltersBar value={dashF} setValue={setDashF} />
                ...
                {dashRoomData.map((r, i) => (
                  <div key={r.id} style={{ marginBottom: "14px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px", fontSize: "14px" }}>
                      <span>
                        {lang === "ar" ? "قاعة" : "Room"} {r.code} ({r.trainer})
                      </span>
          ...
          {tab === "daily" && (
            <div>
              ...
              <FiltersBar value={dailyF} setValue={setDailyF} />
          ...
          {tab === "final" && (
            <div>
              ...
              <FiltersBar value={finalF} setValue={setFinalF} />
          ...
          {tab === "participants" && (
            <div>
              ...
              <FiltersBar value={partF} setValue={setPartF} withQuery />
              ...
              <div className="card noprint" style={{ padding: 16, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <button className="btn2" onClick={() => setRevealPII((v) => !v)} style={{ width: "auto", paddingInline: 16 }}>
                    🔒 {revealPII ? t.hide : t.reveal}
                  </button>
                  <button
                    className="btn2"
                    onClick={() => {
                      // Export all filtered participants (flatten)
                      const flat = [];
                ...
                <div className="badge" style={{ background: "#f1f5f9", color: "#0f172a" }}>
                  {lang === "ar" ? "قاعات مع نتائج" : "Rooms"}: {partRoomData.length}
                </div>
              </div>
                ...
                <div key={r.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
                  <div style={{ background: "#f8fafc", padding: "16px", borderBottom: "1px solid #e2e8f0", display: "flex", gap: "16px", alignItems: "center" }}>
                    <b style={{ fontSize: "18px", color: "#10b981" }}>
                      {lang === "ar" ? "قاعة" : "Room"} {r.code}
                    </b>
                    <span style={{ color: "#64748b" }}>
                      {lang === "ar" ? "المدرب" : "Trainer"}: {r.trainer}
                    </span>
                    <span
                      style={{
                        background: "#dbeafe",
          ...
          {tab === "cert" && (
            <div>
              ...
              <FiltersBar value={certF} setValue={setCertF} withMin />
              ...
              {certBest ? (
                <div className="card" style={{ textAlign: "center", padding: "40px", background: "linear-gradient(135deg, #fffbeb, #fef3c7)", border: "4px solid #d97706" }}>
                  ...
                  <button
                    ...
                    style={{ width: "auto", paddingInline: 24, marginTop: 24, background: "#0f172a" }}
                  >
                    🖨️ {t.print}
                  </button>
                </div>
