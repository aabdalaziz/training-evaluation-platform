  ...
  return <div className="print-axis-all">
    ...
    {items.map((item,i)=>{
      ...
      return <article key={item.axisLabel} className="print-axis-card">
        ...
        <div className="print-axis-chart"><div className="print-chart-title">{isAr?"الدليل الإحصائي: توزيع درجات المحور":"Statistical evidence: rating distribution"}</div>{(item.dist||[0,0,0,0,0]).map((n,j)=>{const pct=item.measurements?Math.round(n/item.measurements*100):0;return <div className="print-chart-row" key={j}><span>{j+1}/5</span><div className="print-chart-track"><i style={{width:`${pct}%`,background:j<2?"#dc2626":j===2?"#d97706":"#0d9488"}}/></div><b>{n} · {pct}%</b></div>})}<div className="print-target-row"><span>{isAr?"مقارنة بالهدف 4.20/5":"Benchmark 4.20/5"}</span><div className="print-chart-track"><i style={{width:`${Math.min(100,item.mean/5*100)}%`,background:item.mean>=TARGET?"#059669":"#d97706"}}/></div><b>{item.mean.toFixed(2)}/5</b></div></div>
...
.print-axis-all{display:none}.print-axis-card{border:1px solid #cbd5e1;border-radius:8px;padding:14px;margin:12px 0;background:#fff}.print-axis-head{display:flex;justify-content:space-between;border-bottom:1px solid #e2e8f0;padding-bottom:8px;color:#0f3b63}.print-axis-kpis{display:flex;gap:14px;flex-wrap:wrap;font-size:12px;margin:10px 0}.print-axis-card p{font-size:13px;line-height:1.8;margin:7px 0;color:#334155}.print-axis-chart{margin:12px 0;padding:12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px}.print-chart-title{font-weight:900;color:#173a5e;margin-bottom:8px}.print-chart-row,.print-target-row{display:grid;grid-template-columns:65px 1fr 70px;gap:8px;align-items:center;margin:7px 0;font-size:12px}.print-chart-track{height:10px;background:#e2e8f0;border-radius:8px;overflow:hidden}.print-chart-track i{display:block;height:100%;border-radius:8px}@media print{.side,.noprint{display:none !important;}.rw{padding:0;background:#fff;}.card,.verdict,.rep-meta{box-shadow:none !important;break-inside:avoid;}.print-axis-all{display:block;break-before:page}.print-axis-card{break-inside:avoid;}}
`;
