...
// ==========================================
// الأنماط البصرية + هندسة الطباعة الاحترافية (Print Media)
// ==========================================
const CSS = `
...
/* 🖨️ هندسة الطباعة (تصدير PDF احترافي) */
@media print {
  @page { size: A4; margin: 15mm; }
  body { background: #fff !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .rw { background: #fff !important; padding: 0 !important; }
  .side, .np, .print-hide { display: none !important; }
...
export default function ReportsPage() {
  ...
  const fetchAllData = async () => {
    try {
      const [e, a, q, progs, roomsList, trainersList] = await Promise.all([
        ...
        db.from("programs").select("*").order("name_ar"),
  ...
  const teacherPerformance = useMemo(() => {
    return (trainers || []).map(t => {
      ...
      const clarityScores: number[] = []; const teachScores: number[] = []; const driveScores: number[] = [];
      (ans || []).forEach(a => {
      ...
      return { id: t.id, name: t.name, rooms: myRooms.map(r => r.code || "").filter(Boolean).join("، ") || "لم تُحدد قاعة", levels: Array.from(new Set(myRooms.map(r => r.level).filter(Boolean))).join("، ") || "—", count: myEvals.length, avg: avg(overall), clarity: avg(clarityScores), teach: avg(teachScores), drive: avg(driveScores) };
  ...
  // ==========================================
  // وظائف التصدير (إكسل وطباعة PDF)
  // ==========================================
  const handleExportCSV = () => {
    let csv = '\uFEFF'; // لدعم اللغة العربية في الإكسل
    ...
    csv += "\n--- أداء المعلمين ---\nالمعلم,القاعة,الاستجابات,وضوح الشرح,أسلوب التدريس,تفاعل الطلاب,المعدل العام\n";
    teacherPerformance.forEach(t => { csv += `"${t.name}","${t.rooms}",${t.count},${t.clarity.toFixed(2)},${t.teach.toFixed(2)},${t.drive.toFixed(2)},${t.avg.toFixed(2)}\n`; });

    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `تقرير_الأداء_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  if (!mounted || load) {
    return (
      <div className="rw" style={{ background: "#f2ecdf", minHeight: "100vh", padding: "80px", textAlign: "center", color: "#64748b" }}>
        <style dangerouslySetInnerHTML={{ __html: CSS }} />
        <div style={{ width: "48px", height: "48px", borderWidth: "5px", borderStyle: "solid", borderColor: "#e2e8f0", borderTopColor: "#10b981", borderRadius: "50%", animation: "spin 1s linear infinite", margin: "0 auto 16px" }}></div>
        <p>جاري تحميل لوحة الأداء والامتثال...</p>
      </div>
    );
  }
    ...
    <div className="rw">
      ...
      <div className="lay">
        ...
        <div className="main">
          ...
          {/* شريط أدوات التصدير - لا يظهر في الطباعة */}
          <div className="print-hide" style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginBottom: "16px" }}>
            <button onClick={handleExportCSV} style={{ background: "#2563eb", color: "#fff", border: "none", padding: "10px 16px", borderRadius: "10px", fontWeight: "bold", cursor: "pointer", fontSize: "13px", display: "flex", gap: "6px", alignItems: "center" }}>
              📊 تصدير Excel
            </button>
            <button onClick={() => window.print()} style={{ background: "#0f172a", color: "#fff", border: "none", padding: "10px 16px", borderRadius: "10px", fontWeight: "bold", cursor: "pointer", fontSize: "13px", display: "flex", gap: "6px", alignItems: "center" }}>
              🖨️ طباعة / حفظ كـ PDF
            </button>
          </div>
          ...
          <div className="hero-print" style={{ position: "relative", overflow: "hidden", background: "linear-gradient(135deg,#0b1220,#111827)", borderRadius: "24px", padding: "24px", color: "#fff", marginBottom: "16px" }}>
            ...
            <div style={{ position: "relative" }}>
              <span style={{ background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.12)", borderRadius: "999px", padding: "4px 12px", fontSize: "11px", fontWeight: "700", color: "#e2e8f0", display: "inline-block", marginBottom: "6px" }}>وثيقة رسمية للإدارة الأكاديمية</span>
  ...
  return (
    <div>
      <div className="k3" style={{ marginBottom: "16px" }}>
        <KpiCard label="إجمالي الاستجابات החية" value={Number(d.count || 0) + Number(f.count || 0)} suffix=" استمارة" ghost={String(Number(d.count || 0) + Number(f.count || 0))} />
      ...
      <div className="g2">
        ...
        {p.best ? (
          <div className="card" style={{ background: "linear-gradient(135deg, #0f172a, #1e293b)", color: "#fff", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", border: "1px solid #10b981", padding: "22px" }}>
          ...
          <div className="card"><p style={{ color: "#94a3b8", textAlign: "center", padding: "40px" }}>لا توجد تقييمات مسجلة للمدربين بعد.</p></div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="g2" style={{ marginBottom: "16px" }}>
        <div className="card">
          <h3 style={{ margin: "0 0 12px", fontSize: "16px", fontWeight: "800" }}>🎯 توزيع التقييم الإجمالي</h3>
          {[5, 4, 3, 2, 1].map(s => {
            const idx = s - 1;
      ...
      <div className="g2" style={{ marginBottom: "16px" }}>
        ...
        <div className="card">
          <h3 style={{ margin: "0 0 12px", fontSize: "16px", fontWeight: "800" }}>⚖️ مقارنة الأقسام</h3>
          {(r.sections || []).length > 0 ? (
      ...
      <div className="card" style={{ marginBottom: "16px" }}>
        <h3 style={{ margin: "0 0 12px", fontSize: "16px", fontWeight: "800" }}>📈 أداء المحاور التفصيلي</h3>
        {(r.axes || []).length > 0 ? r.axes.map((a, i) => {
          if (!a) return null;
          const s = statusOf(a.value);
          return (
            <div key={i} style={{ background: "#fff", border: "1px solid #f0e9db", borderRadius: "14px", padding: "13px", marginBottom: "10px", pageBreakInside: "avoid" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px", flexWrap: "wrap", gap: "6px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}><span style={{ background: "#f0e9db", color: "#7c6f5a", borderRadius: "7px", padding: "2px 9px", fontSize: "11px", fontWeight: "700" }}>{a.section}</span><span style={{ fontSize: "13.5px", fontWeight: "700" }}>{a.label}</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}><span style={{ background: s.bg, color: s.fg, borderRadius: "999px", padding: "3px 11px", fontSize: "11px", fontWeight: "800" }}>{s.t}</span><b style={{ minWidth: "50px", textAlign: "left", color: p.accent, fontSize: "15px" }}>{Number(a.value || 0).toFixed(2)}/5</b></div>
              </div>
              <div style={{ height: "10px", background: "#f0e9db", borderRadius: "8px", overflow: "hidden" }}><div style={{ width: (Number(a.value || 0) / 5 * 100) + "%", height: "100%", background: "linear-gradient(90deg," + p.accent + ",#10b981)", borderRadius: "8px" }} /></div>
            </div>
          );
        }) : <p style={{ color: "#9a8f7d", textAlign: "center", padding: "20px" }}>لا توجد بيانات لهذا التبويب حالياً.</p>}
      </div>

      <div className="card" style={{ marginTop: "16px", pageBreakInside: "avoid" }}>
        <h3 style={{ margin: "0 0 12px", fontSize: "16px", fontWeight: "800" }}>💬 ملاحظات المشاركين</h3>
        {(r.comments || []).length > 0 ? r.comments.map((c, i) => (
          <div key={i} style={{ background: "#fff", borderRight: "4px solid " + p.accent, borderRadius: "12px", padding: "12px", marginBottom: "8px", fontSize: "13px", color: "#475569", lineHeight: "1.7", pageBreakInside: "avoid" }}>{c}</div>
        )) : <p style={{ color: "#9a8f7d", textAlign: "center", padding: "20px" }}>لا توجد تعليقات.</p>}
      </div>
    </div>
  );
}
...
function TeachersView(p: { data: any[]; best: any }) {
  ...
  return (
    <div>
      <div className="card">
        <h3 style={{ margin: "0 0 14px", fontSize: "16px", fontWeight: "800" }}>📋 ترتيب المعلمين والأداء الأكاديمي</h3>
...
function KpiCard(p: { label: string; value: any; suffix?: string; ghost: string }) {
  return (
    <div style={{ position: "relative", overflow: "hidden", borderRight: "4px solid #10b981", background: "#fffdf9", borderTop: "1px solid #e8decb", borderBottom: "1px solid #e8decb", borderLeft: "1px solid #e8decb", borderRadius: "22px", padding: "22px" }}>
      <div style={{ position: "absolute", left: "10px", top: "4px", fontSize: "60px", fontWeight: "900", color: "rgba(16,185,129,0.04)" }}>{p.ghost || ""}</div>
      <div style={{ position: "relative" }}>
        <div style={{ fontSize: "13px", color: "#9a8f7d", fontWeight: "700", marginBottom: "4px" }}>{p.label || ""}</div>
        <div style={{ fontSize: "38px", fontWeight: "900", color: "#0f172a", lineHeight: "1" }}>{p.value}{p.suffix || ""}</div>
      </div>
    </div>
  );
}
