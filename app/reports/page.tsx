...
export default function Page() {
  ...
  const banner = (<div style={{ background: "#facc15", color: "#111", padding: "8px 14px", borderRadius: 10, fontWeight: 900, marginBottom: 14, textAlign: "center", fontSize: 13 }}>V-REPORT-1 • لوحة التقارير (مرحلة 1)</div>);
  if (load) return (<div dir="rtl" style={{ background: "#f1ece1", minHeight: "100vh", padding: 14, fontFamily: "Cairo,Tahoma,sans-serif" }}>{banner}<div style={{ textAlign: "center", color: "#64748b", padding: 40 }}>جارٍ تحميل البيانات...</div></div>);
  ...
  return (
    <div dir="rtl" style={{ background: "#f1ece1", minHeight: "100vh", padding: 14, fontFamily: "Cairo,Tahoma,sans-serif" }}>
      {banner}
          ...
          {tab !== "teachers" && (<>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div style={{ background: "#fffdf9", borderRadius: 16, padding: 14, border: "1px solid #ece4d4" }}>
                <h3 style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 800 }}>📊 التوزيع اليومي (7 أيام)</h3>
                <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 130 }}>
                  {days.map((day, i) => (<div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }}>
                    <span style={{ fontSize: 11, fontWeight: 800, marginBottom: 4 }}>{day.count}</span>
                    <div style={{ width: "70%", height: Math.max(6, (day.count / maxDay) * 95) + "px", background: "linear-gradient(180deg,#34d399,#059669)", borderRadius: "6px 6px 0 0" }} />
                    <span style={{ fontSize: 9, color: "#64748b", marginTop: 4 }}>{day.wd}</span>
                  </div>))}
                </div>
              </div>
              <div style={{ background: "#fffdf9", borderRadius: 16, padding: 14, 
